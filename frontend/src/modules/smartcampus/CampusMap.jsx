...
import { MapPin, Navigation, Search, Building, BookOpen, Cpu, Coffee, Music, Info } from 'lucide-react';
...
// Helper component to recenter map when selected
...
export default function CampusMap() {
  ...
  const [mapCenter, setMapCenter] = useState([23.7772, 90.3995]); // Default campus center

  useEffect(() => {
    // Fetch campus locations from API or fallback
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '20px', minHeight: '600px' }}>
      ...
      {/* Map View & Location Details Drawer */}
      <div style={{ position: 'relative', borderRadius: '16px', overflow: 'hidden', border: '1px solid #334155', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* Floating Selected Location Info Drawer */}
        {selectedLocation && (
          <div style={{
            position: 'absolute',
            bottom: '16px',
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>{selectedLocation.building_no}</span>
              </div>
              <h4 style={{ margin: '4px 0', fontSize: '1.05rem', color: '#fff', fontWeight: '700' }}>{selectedLocation.name}</h4>
              <p style={{ margin: 0, fontSize: '0.8rem', color: '#cbd5e1', lineHeight: '1.4' }}>{selectedLocation.description}</p>
            </div>
          </div>
        )}
      </div>
